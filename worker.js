...
async function processUploadJob(j,e){
 ...
 // RAM cache is not reliable across Cloudflare isolates. Recover the panel
 // message id from KV so every upload on the SAME channel edits one message.
 if(!p){
 ...
 let panelState={};
 try{panelState=JSON.parse(await e.BOT_STATE.get("panel_state")||"{}")}catch{}
 const savedChannel=panelState.channel||"";
 const savedPanelChat=panelState.chat||"";
 const savedPanelMsg=panelState.msg||"";

 // Same channel + same private bot chat = reuse the existing panel message.
 if(String(savedChannel)===String(activeChannel) &&
    String(savedPanelChat)===String(panelChat) && savedPanelMsg){
  p.msg_id=Number(savedPanelMsg);
 }

 // No saved panel (or channel changed) = create one and remember its id.
 if(!p.msg_id){
  ...
  if(sent?.ok){
   p.msg_id=sent.result.message_id;
   // One KV write ONLY when a NEW panel is created.
   // Normal uploads: 1 KV READ, 0 KV WRITE.
   await e.BOT_STATE.put("panel_state",JSON.stringify({channel:String(activeChannel),chat:String(panelChat),msg:String(p.msg_id)}));
  }
 }
