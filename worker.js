...
/* RAM ONLY: panel state never goes to KV */
const MEM_PANEL=new Map();
   ...
   try{
    ...
    if(e.message==="__PAUSED__"){
     ...
     if(msg.attempts>3){
      ...
      await sendMessage(
       env,
       j.auto?(j.panel_chat||j.chat_id):j.chat_id,
       text,
       j.auto?{disable_notification:true}:{
...
📡 Auto upload: RAM Live Panel`);
 ...
 if(["/addadmin","/removeadmin","/admins"].includes(cmd)){
  if(!own)
   return sendMessage(e,chat,"❌ Sirf owner ye command use kar sakta hai.");
 ...
 if(cmd==="/setfolder"){
  ...
  if(!f)
   return sendMessage(e,chat,"❌ Invalid folder path.");
 ...
 if(cmd==="/setchannel"){
  if(!arg)
   return sendMessage(e,chat,"Use /setchannel CHANNEL_ID");
 ...
 if(cmd==="/auto"){
  ...
  if(v!=="on"&&v!=="off")
   return sendMessage(e,chat,"Use /auto on or /auto off");
 ...
 if(cmd==="/delete")
  return deleteCommand(e,chat,arg);
...
/* =========================================================
   UPLOAD QUEUE
   Upload receive:
   - current folder = 1 KV READ
   - no folder KV WRITE
   - auto panel owner lookup = 1 extra KV READ only for auto
   ========================================================= */
...
async function enqueueUpload(doc,chat,uid,e,auto,msg_id){
 ...
 if(!name){
  if(!auto)
   await sendMessage(e,chat,"❌ Sirf .html ya .json file allowed hai.",
 ...
 if((doc.file_size||0)>MAX_FILE_SIZE){
  if(!auto)
   await sendMessage(e,chat,"❌ Maximum file size 20 MB hai.",
    {reply_to_message_id:msg_id});
  return;
 }
...
/* =========================================================
   RAM LIVE PANEL
   No KV writes.
   Auto channel uploads show live progress in owner's chat.
   ========================================================= */

function getPanel(key,folder,auto){
 const now=Date.now();
 let p=MEM_PANEL.get(key);

 if(!p||now-p.time>PANEL_TTL){
  p={
   msg_id:null,
   folder,
   auto,
...
function panelText(p){
 return `📊 <b>SMART RAM LIVE PANEL</b>${p.auto?" 📡 AUTO":""}
...
⚙️ <i>Processing:</i>
📄 <code>${escapeHtml(p.current||"Waiting...")}</code>`;
}

async function panelStart(e,p,chat){
 if(p.msg_id)return;

 const r=await tgCall(e,"sendMessage",{
  chat_id:chat,
  text:panelText(p),
  parse_mode:"HTML"
 });

 if(r?.ok)p.msg_id=r.result.message_id;
}

async function panelUpdate(e,p,chat,current){
 p.current=current||"Waiting...";
 p.time=Date.now();

 if(!p.msg_id){
  await panelStart(e,p,chat);
  return;
 }

 await tgCall(e,"editMessageText",{
  chat_id:chat,
  message_id:p.msg_id,
  text:panelText(p),
  parse_mode:"HTML"
...
async function processUploadJob(j,e){
 if(await isStopped(e,j.chat_id)&&!j.auto)
  throw Error("__PAUSED__");

 let p=null;
 const panelChat=j.panel_chat||j.chat_id;

 if(j.auto){
  p=getPanel(panelChat,j.folder,true);
  p.total++;
  await panelUpdate(e,p,panelChat,`📥 ${j.file_name}`);
 }

 let r;

 try{
  r=await processDocument(j,e,async stage=>{
   if(j.auto&&p)
    await panelUpdate(e,p,panelChat,stage);
  });
 }catch(x){
 ...
 if(j.auto&&p){
  ...
  await panelUpdate(
   e,p,panelChat,
   r.ok
    ?(r.skipped?`⏭️ Skipped: ${j.file_name}`:`✅ Done: ${j.file_name}`)
    :`❌ Failed: ${j.file_name}`
  );
 }
 ...
 if(!j.auto){
  ...
  await sendMessage(e,j.chat_id,text,{
   reply_to_message_id:j.msg_id,
   disable_notification:true
  });
 }
...
async function processDocument(j,e,progress=async()=>{}){
 if(await isStopped(e,j.chat_id)&&!j.auto)
  throw Error("__PAUSED__");

 const isDirectJson=/\.json$/i.test(j.file_name);

 const targetJson=isDirectJson
  ?j.file_name
  :j.file_name.replace(/\.(html|htm)$/i,".json");

 await progress(`🔎 Checking GitHub: ${targetJson}`);

 const existing=await githubFileExists(
  e,`${j.folder}/${targetJson}`
 );

 if(!existing.ok)
  throw Error("GitHub existing-file check failed: "+existing.message);

 if(existing.exists)
  return{ok:true,skipped:true,questions:0,jsonName:targetJson};

 await progress(`📥 Downloading: ${j.file_name}`);

 const f=await telegramGetFile(e,j.file_id);

 if(!f.ok)
  throw Error("Telegram file lookup failed.");
 ...
 if(!r.ok)
  throw Error("Telegram download failed.");

 const buf=await r.arrayBuffer();

 if(buf.byteLength>MAX_FILE_SIZE)
  throw Error("File 20 MB se badi hai.");

 const text=new TextDecoder().decode(buf);

 if(isDirectJson){
  await progress(`🧪 Validating JSON: ${j.file_name}`);

  let data;

  try{data=JSON.parse(text);}
  catch(x){throw Error("Invalid JSON: "+x.message);}

  if(!Array.isArray(data))
   throw Error("JSON root array [...] hona chahiye.");

  await progress(`☁️ Uploading JSON: ${j.file_name}`);

  const g=await githubPut(
   e,`${j.folder}/${j.file_name}`,
   textToBase64(text),
 ...
 await progress(`🧠 Parsing HTML: ${j.file_name}`);
 ...
 await progress(`☁️ Uploading JSON: ${json}`);

 const g=await githubPut(
  e,
  `${j.folder}/${json}`,
  textToBase64(JSON.stringify(q.questions,null,2)),
  `Add JSON ${json}`
 );
...
async function deleteCommand(e,chat,arg){
 ...
 if(isNumbers){
  ...
  if(!r.ok)
   return sendMessage(e,chat,`❌ Folder nahi mila:\n${folder}`);

  const files=r.items
   .filter(x=>x.type==="file"&&/\.(html?|json)$/i.test(x.name))
  ...
  if(sel.some(n=>n<1||n>files.length))
   return sendMessage(e,chat,`❌ Number 1-${files.length} ke beech hona chahiye.`);
...
/delete 1 3 5`;
 ...
 if(listFolder!==folder){
  footer=`⚠️ Ye current active folder nahi hai.
Pehle isko set karo:
...
KV:
• Upload folder = 1 READ
• Upload = 0 WRITE
• Folder/channel/auto/admin/stop changes = KV WRITE
• Auto progress = RAM ONLY 🚀`);
}
...
/* =========================================================
   ROBUST HTML -> QUIZ JSON
   Supports Qs / Q_DATA JavaScript array literals.
   This replaces the old fragile sanitizeJS().
   ========================================================= */
...
function htmlToQuizJson(s){
 ...
 for(const re of names){
  ...
  if(!a)
   return{ok:false,message:"Question array complete nahi mili."};

  try{
   const q=new JSValueParser(a).parse();
   ...
   return{
    ok:false,
    message:`Qs/Q_DATA parse error: ${String(err.message||err)}`
   };
  }
 }
...
class JSValueParser{
 ...
 error(msg){
  throw Error(`${msg} near character ${this.i}`);
 }
 ...
 ws(){
  while(this.i<this.n){
   ...
   if(c==="/"&&this.s[this.i+1]==="*"){
    ...
    if(end<0)this.error("Unclosed comment");
    this.i=end+2;
    continue;
   }
 ...
 parse(){
  ...
  const v=this.value();
  this.ws();
  if(this.i<this.n)this.error("Unexpected data");
  return v;
 }
 ...
 object(){
  ...
  while(true){
   ...
   if(this.s[this.i]!==":")
    this.error(`Expected : after key "${key}"`);
 ...
 string(){
  ...
  while(this.i<this.n){
   ...
   if(this.i>=this.n)this.error("Unclosed string");
 ...
 identifier(){
  ...
  if(w==="NaN")return null;
  if(w==="Infinity")return null;

  /* Common JS values that may occur in quiz data */
  if(w==="true")return true;
  if(w==="false")return false;

  /* Unknown bare identifier: preserve it as text rather than
     destroying the whole question set. */
  return w;
}
 ...
 for(let i=1;i<=RETRIES;i++){
  try{
   ...
   if(r.status===429||r.status>=500){
    if(i===RETRIES)
     return{ok:false,exists:false,message:`GitHub check failed (${r.status})`};
   ...
   return{
    ok:false,
    exists:false,
    message:(await r.text()).slice(0,300)
   };

  }catch(x){
   if(i===RETRIES)
    return{ok:false,exists:false,message:x.message};
 ...
 for(let i=1;i<=RETRIES;i++){
  try{
   ...
   if(x.ok)sha=(await x.json()).sha;
    ...
    return{ok:false,message:(await x.text()).slice(0,300)};
   ...
   if(![409,422].includes(r.status)||i===RETRIES)
    return{ok:false,message:(await r.text()).slice(0,300)};
...
async function githubDelete(e,path,message){
 ...
 for(let i=1;i<=RETRIES;i++){
  try{
   ...
   if(x.status===404)
    return{ok:false,message:"File GitHub par nahi mila."};
...
async function sendMessage(e,chat,text,options={}){
 try{
  await tgCall(e,"sendMessage",{
   chat_id:chat,
   text,
   ...options
  });
 }catch(x){
  console.error("TG SEND",x);
 }
}
...
async function fetchTimeout(url,opt,ms){
 ...
 try{
  ...
  if(x.name==="AbortError")
   throw Error(`Request timeout (${ms/1000}s)`);
  throw x;
 }finally{
  clearTimeout(t);
 }
