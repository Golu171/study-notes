...
⚡ Uploads do NOT write to KV`);
 ...
 if(cmd==="/mkdir"){
  const f=cleanPath(arg);
  if(!f)return sendMessage(e,chat,"❌ Invalid folder path.");
  // GitHub folders are virtual. Re-running /mkdir on an existing folder
  // must never create another logical folder or overwrite its marker.
  const marker=await githubFileExists(e,`${f}/.gitkeep`);
  if(marker.ok&&marker.exists)return sendMessage(e,chat,`✅ Folder already exists:
${f}`);
  const listed=await githubList(e,f);
  if(listed.ok&&listed.items.length)return sendMessage(e,chat,`✅ Folder already exists:
${f}`);
...
${f}`:`❌ ${r.message}`);
 ...
 if(cmd==="/setchannel"){
  ...
  const newChannel=String(arg).trim();
  const oldChannel=await e.BOT_STATE.get("channel_id");
  if(String(oldChannel||"")!==newChannel){
   const pc=await e.BOT_STATE.get("panel_chat_id");
   const ps=await e.BOT_STATE.get("panel_state");
   try{
    const old=JSON.parse(ps||"{}");
    if(pc&&old.msg&&old.chat&&String(old.chat)===String(pc)){
     await tgCall(e,"deleteMessage",{chat_id:pc,message_id:Number(old.msg)});
    }
   }catch{}
   if(pc)delete MEM_CACHE[`${String(oldChannel||"")}|${String(pc)}`];
   await e.BOT_STATE.put("panel_state","");
  }
  await e.BOT_STATE.put("channel_id",newChannel);
  if((await e.BOT_STATE.get("auto"))==="on"&&m.chat?.type==="private"){
   const pc=await e.BOT_STATE.get("panel_chat_id")||chat;
   await e.BOT_STATE.put("panel_chat_id",pc);
   await ensureLivePanel(e,newChannel,pc);
  }
...
${newChannel}`);
 ...
 if(cmd==="/auto"){
  ...
  // Auto/panel control is anchored only from the private bot chat.
  // Do not change the persisted AUTO state when someone runs /auto on
  // inside a channel or group by mistake.
  if(v==="on"&&m.chat?.type!=="private"){
...
async function processDocument(j,e){
 ...
 if(isDirectJson){
  /*
    IMPORTANT: Direct JSON uploads must be byte-for-byte content-preserving.
    The old code parsed + normalized every JSON file, which rebuilt each
    question object and could change answer/option representations.
    For a .json upload, validate the JSON but write the ORIGINAL text.
  */
  let raw;
  try{
   raw=JSON.parse(String(text||"").replace(/^\uFEFF/,""));
  }catch(x){
   throw Error("Invalid JSON: "+x.message);
  }
  const out=j.file_name.replace(/\.json$/i,".json");
  const g=await githubPut(e,`${j.folder}/${out}`,textToBase64(text),`Add JSON ${out}`);
  if(!g.ok)throw Error(g.message);
  const list=Array.isArray(raw)?raw:(raw&&typeof raw==="object"?findQuestionList(raw):[]);
  return{ok:true,questions:Array.isArray(list)?list.length:0,jsonName:out};
 }
...
function htmlToQuizJson(s){
 const keys=[
  /const\s+Qs\s*=/i,/let\s+Qs\s*=/i,/var\s+Qs\s*=/i,
  /const\s+Q_DATA\s*=/i,/let\s+Q_DATA\s*=/i,/var\s+Q_DATA\s*=/i
 ];
 ...
 /*
   Fallback for HTML files whose question data is not named Qs/Q_DATA.
   Scan JavaScript variable assignments such as:
     const questions = [...]
     const quizData = [...]
     let data = [...]
   and select the first parsed candidate that actually contains MCQ-like
   question objects. This fixes "Qs / Q_DATA nahi mila" without requiring
   the HTML generator to use one hard-coded variable name.
 */
 const assign=/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*([\[{])/g;
 let m,checked=0;
 while((m=assign.exec(s))!==null && checked<80){
  const open=m[2],close=open==="["?"]":"}";
  const a=extractBalanced(s,m.index+m[0].length,open,close);
  if(!a)continue;
  checked++;
  try{
   const raw=parseJSArray(a);
   const list=findQuestionList(raw);
   if(Array.isArray(list)&&list.length){
    const norm=list.map((x,i)=>normalizeQuestion(x,i)).filter(x=>x.question||x.options.some(Boolean)||x.solution);
    if(norm.length)return{ok:true,questions:list};
   }
  }catch{}
 }
 return{ok:false,message:"Question data nahi mila. Qs/Q_DATA ya recognizable question-data array nahi mili."};
}
