  ...
  for(const msg of batch.messages){
   ...
   try{
    ...
    if(e.message==="__PAUSED__"){
     ...
     if(msg.attempts>3){
      msg.ack();
      await sendMessage(env,j.chat_id,
       `❌ Failed: ${j.file_name||j.name}\nReason: ${e?.message||String(e)||"Unknown queue error"}`,
       {reply_to_message_id:j.msg_id,disable_notification:true});
...
async function handleChannelPost(m,e){
 ...
 await enqueueUpload(m.document,String(m.chat.id),String(m.sender_chat?.id||m.chat.id),e,true,m.message_id,panelChat);
}
...
async function processUploadJob(j,e){
 ...
 const now=Date.now();
 let p=MEM_CACHE[panelChat];
 // RAM panel: keep the same panel for this Worker isolate instead of
 // resetting it every 5 minutes. This prevents a new panel message
 // during a long upload run. No per-upload KV write is added.
 if(!p){
  p={msg_id:null,done:0,skip:0,fail:0,time:now};
  MEM_CACHE[panelChat]=p;
 }
 p.time=now;

 if(!p.msg_id){
  const sent=await tgCall(e,"sendMessage",{
   chat_id:panelChat,
   text:`📊 <b>SMART LIVE PANEL</b> ${j.auto?"(Channel Auto)":""}\n📁 <code>${escapeHtml(j.folder)}</code>\n\n⏳ <i>Starting process...</i>`,
   parse_mode:"HTML"
  });
  if(sent?.ok)p.msg_id=sent.result.message_id;
 }
...
async function updatePanel(e,j,p,extra){
 ...
 await tgCall(e,"editMessageText",{chat_id:panelChat,message_id:p.msg_id,text,parse_mode:"HTML"});
}
...
async function processDocument(j,e){
 ...
 if(isDirectJson){
  let data;
  try{data=JSON.parse(text)}catch(x){throw Error("Invalid JSON: "+x.message)}
...
/*
  Converts the common JS object/array syntax used by the study HTML files
  without corrupting escaped newlines, quotes, apostrophes or backticks.
*/
function parseJSArray(src){
 // First accept the exact JSON format used by many exported HTML files.
 // This also handles pretty-printed Qs arrays with newlines/indentation.
 ...
 // Fallback: parse JavaScript-style Qs arrays (single quotes, template
 // strings, unquoted keys, comments, undefined, trailing commas, etc.).
 let out="",i=0;
 while(i<src.length){
  const c=src[i];
  ...
  if(c==='"'){
   const r=readJSString(src,i,'"');out+=JSON.stringify(r.value);i=r.next;continue;
  }
  if(c==="'"){
   const r=readJSString(src,i,"'");out+=JSON.stringify(r.value);i=r.next;continue;
  }
  if(c==="`"){
   const r=readTemplateString(src,i);out+=JSON.stringify(r.value);i=r.next;continue;
  }
  out+=c;i++;
 }
 out=out.replace(/([{,]\s*)([A-Za-z_$][\w$]*)\s*:/g,'$1"$2":');
 out=out.replace(/\bundefined\b/g,"null").replace(/([{,]\s*)!0\b/g,'$1true').replace(/([{,]\s*)!1\b/g,'$1false');
 out=out.replace(/,\s*([}\]])/g,"$1");

 // JSON.stringify() above guarantees that control characters inside parsed
 // strings are escaped, so JSON.parse() is safe even for HTML containing
 // line breaks, tabs or CR/LF inside the original JavaScript strings.
 try{return JSON.parse(out)}
 catch(err){
  const pos=Number(err.message.match(/position (\d+)/i)?.[1]||-1);
  const around=pos>=0?out.slice(Math.max(0,pos-45),pos+75).replace(/\s+/g," "):"";
  throw Error(`${err.message}${around?` | near: ${around}`:""}`);
 }
}

function readJSString(s,start,quote){
 let i=start+1,v="";
 while(i<s.length){
  const c=s[i++];
  if(c===quote)return{value:v,next:i};
  if(c!=="\\"){v+=c;continue}
  if(i>=s.length)break;
  const n=s[i++];
  ...
  if(n==="u"){
   ...
   if(/^[0-9a-fA-F]{4}$/.test(h)){v+=String.fromCharCode(parseInt(h,16));i+=4;continue}
  }
  if(n==="x"){
   const h=s.slice(i,i+2);
   if(/^[0-9a-fA-F]{2}$/.test(h)){v+=String.fromCharCode(parseInt(h,16));i+=2;continue}
  }
  v+=n;
 }
...
function readTemplateString(s,start){
 let i=start+1,v="";
 while(i<s.length){
  const c=s[i++];
  if(c==="`")return{value:v,next:i};
  if(c!=="\\"){v+=c;continue}
  if(i>=s.length)break;
  const n=s[i++];
  if(n==="n")v+="\n";
