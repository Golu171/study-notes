...
async function enqueueUpload(doc,chat,uid,e,auto,msg_id){
 ...
 const j={type:"upload",chat_id:chat,file_id:doc.file_id,file_name:name,folder,auto,msg_id};
...
async function processUploadJob(j,e){
 ...
 const now=Date.now();
 let p=MEM_CACHE[j.chat_id];
 if(!p||(now-p.time>5*60*1000)){
  p={msg_id:null,done:0,skip:0,fail:0,time:now};
  MEM_CACHE[j.chat_id]=p;
 }
 p.time=now;
 ...
 if(!r.ok){
  await sendMessage(e,j.chat_id,`❌ Failed: ${j.file_name}\nReason: ${r.message}`,{reply_to_message_id:j.msg_id,disable_notification:true});
 }
 await sleep(1000);
}
...
async function updatePanel(e,j,p,extra){
 ...
 await tgCall(e,"editMessageText",{chat_id:j.chat_id,message_id:p.msg_id,text,parse_mode:"HTML"});
}
