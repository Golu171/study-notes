...
async function enqueueUpload(doc,chat,uid,e,auto,msg_id,panelChat=null){
 ...
 const channelId=auto ? String(chat) : String(await e.BOT_STATE.get("channel_id")||"");
 const j={type:"upload",chat_id:chat,panel_chat_id:panelChat||chat,channel_id:channelId,file_id:doc.file_id,file_name:name,folder,auto,msg_id};
...
async function processUploadJob(j,e){
 ...
 if(!p.msg_id){
  ...
  if(sent?.ok){
   ...
   await e.BOT_STATE.put(panelKey,JSON.stringify({channel:String(activeChannel),chat:String(panelChat),msg:String(p.msg_id)}));
  }
 }
